...
def get_input_arguments():
    ...
    try:
        ...
        parser.add_argument('--input_path', dest='input_path', type=str, required=True, help='GCS Input Bucket Path')
        parser.add_argument('--error_path', dest='error_path', type=str, required=True, help='GCS error Bucket Path')
        parser.add_argument('--success_path', dest='success_path', type=str, required=True,
                            help='GCS Success Bucket Path')
        args = parser.parse_args()

        # Assign command-line arguments to variables
        input_path = args.input_path
        error_path = args.error_path
        output_path = args.success_path
        date_missing_files = ast.literal_eval(args.date_missing_files)
...
def has_valid_extension(filename, valid_extensions):
    """
    The has_valid_extension function checks if the file extension is valid.
        Args:
            filename (str): The name of the file to check.
            valid_extensions (list): A list of strings containing all acceptable extensions for a given file type.

    :param filename: Pass the filename to the function
    :param valid_extensions: Pass the list of valid extensions
    """
    try:
        file_extension = filename.split('.')[-1]  # Extract the file extension
        ...
        logger.error(f"Error occurred while checking the valid extension : {e}")
        raise


def match_pattern(filename, input_param):
    """
    The match_pattern function takes in a filename and an input parameter.
    It then creates a regular expression pattern based on the input parameter,
    and uses that to match against the filename. If there is no match, it returns False.

    :param filename: Pass the file name to be matched against the pattern
    :param input_param: Match the file name with the input parameter
    """
    try:
        pattern = rf'^{input_param}-(\d{{8}})-(\d+)$'
        regex = re.compile(pattern)
        match = regex.match(filename)
        return bool(match)
    except Exception as e:
        logger.error(f"Error while matching the file name pattern : {e}")
        raise


def process_each_file_parallely(each_file_uri,
                                ...
                                file_type):
    """
    The process_each_file_parallely function is used to process each file parallely.
        Args:
            each_file_uri (str): The URI of the file to be processed.
            file_name_with_extension (str): The name of the file with extension.
            success_path (str): The path where files are uploaded after processing successfully.

    :param each_file_uri: Get the file path from gcs
    :param file_name_with_extension: Get the file name with extension
    :param file_date: Create a folder with the same name in gcs
    :param path_: Upload the file to gcs
    :param each_reference_file: Create a folder in the destination path
    """
    try:
        # local_path = utils.gcs_utils.download_from_gcs(each_file_uri)
        if file_type == "all_valid":
            destination_path = f"{path_.rstrip('/')}/{each_reference_file}/{file_name_with_extension}"
        elif file_type == "valid_name":
            destination_path = f"{path_.rstrip('/')}/extension_error/{each_reference_file}/{file_name_with_extension}"
        elif file_type == "file_name_error":
            destination_path = f"{path_.rstrip('/')}/file_name_error/{file_name_with_extension}"
        # utils.gcs_utils.upload_file_to_gcs(local_path, destination_path)
        utils.gcs_utils.copy_file(each_file_uri, destination_path)
    except Exception as e:
        logger.error(f"Error occurred while processing each file parallel : {e}")
        raise


def process_all_files(all_files, all_reference_files, valid_extensions, success_path, error_path):
    """
    The process_all_files function is used to process all the files in a given directory.
        It takes in the following parameters:
            1) all_files - A list of file paths for each file that needs to be processed.
            2) all_reference_files - A list of reference files that are used as a basis for processing each file.
                                    The format of these reference files should be file-name;-date;.extension;
                                    where date is in YYYYMMDD format and extension can be any valid extension like 
                                    .csv, .txt etc.

    :param all_files: Pass the list of all files in the source path
    :param all_reference_files: Match the file name with the reference files
    :param valid_extensions: Check if the file has a valid extension
    :param success_path: Move the file to a success folder after processing
    :param error_path: Move the file to a error folder after processing
    """
    try:
        all_valid_files_to_process = []
        all_error_files_to_process = []
        for each_file in all_files:
            logger.info(f"Processing File : {each_file}")
            file_name_with_extension = each_file.split("/")[-1]
            file_name_without_extension = file_name_with_extension.split(".")[0]
            for each_reference_file in all_reference_files:
                if each_reference_file in each_file:
                    has_valid_ext = has_valid_extension(file_name_with_extension, valid_extensions)
                    has_valid_name = match_pattern(file_name_without_extension, each_reference_file)
                    if has_valid_ext and has_valid_name:
                        file_date = file_name_without_extension.split('-')[1]
                        all_valid_files_to_process.append([each_file,
                                                           file_name_with_extension,
                                                           file_date,
                                                           success_path,
                                                           each_reference_file, "all_valid"])
                    elif has_valid_name:
                        file_date = file_name_without_extension.split('-')[1]
                        all_error_files_to_process.append([each_file,
                                                           file_name_with_extension,
                                                           file_date,
                                                           error_path,
                                                           each_reference_file, "valid_name"])
        ...
        # Function to check if any reference string is present in the input string
        def contains_reference(input_string, reference_list):
            return any(ref in input_string for ref in reference_list)

        # Filter out the elements that do not contain any reference string
        error_files_uri = [item for item in all_files if not contains_reference(item, all_reference_files)]
        for error_file_uri in error_files_uri:
            file_name_with_extension = error_file_uri.split("/")[-1]
            file_name_without_extension = file_name_with_extension.split(".")[0]
            all_error_files_to_process.append([each_file,
                                               file_name_with_extension,
                                               "",
                                               ...
                                               "", "file_name_error"])
        pool = helpers.multiprocessing_helpers.get_multiprocessing_pool()
        pool.starmap(process_each_file_parallely, all_valid_files_to_process)
        helpers.multiprocessing_helpers.close_multiprocessing_pool(pool)
        helpers.multiprocessing_helpers.join_multiprocessing_pool(pool)

        pool = helpers.multiprocessing_helpers.get_multiprocessing_pool()
        pool.starmap(process_each_file_parallely, all_error_files_to_process)
        helpers.multiprocessing_helpers.close_multiprocessing_pool(pool)
        helpers.multiprocessing_helpers.join_multiprocessing_pool(pool)
    except Exception as e:
        logger.error(f"Error occurred while processing all files : {e}")
        raise
